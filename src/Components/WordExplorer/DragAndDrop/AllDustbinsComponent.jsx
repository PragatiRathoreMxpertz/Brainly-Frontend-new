      ...
      <div key={index} className="all-dustbin">
        ...
        <div className=''>
          <div className='droppedImg'>
            ...
            <img src={` https://ik.imagekit.io/dev24/${dustbin.Storyimage}`} alt={`Box dropped in Dustbin ${index + 1}` } style={{width:"100%" ,height:"100%"}} />
         
          </div>
        </div>

        <div className=' h-[10%]'>
          <p className='font-poppins font-normal Synonyms'>Synonyms: <span className='text-white'>{dustbin.Synonyms}</span></p>
          <p className='font-poppins font-normal  Antonyms'>Antonyms: <span className='text-white'>{dustbin.Antonyms}</span></p>
        </div>
      </div>
      ))}
    </div>
